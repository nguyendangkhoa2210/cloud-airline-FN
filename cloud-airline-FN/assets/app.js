...
function handleBookingSessionExpired() {
  ...
  // đăng xuất cả.
  if (isLoggedIn) {
    fetch(`${API_BASE}/logout.php`, { method: 'POST', headers: jsonHeaders(), body: '{}' }).catch(() => {});
    isLoggedIn = false;
    userFullName = '';
    userRole = 'user';
  }
...
function setupActionListeners() {
  ...
  if (guestLookupForm) {
    guestLookupForm.addEventListener('submit', async (e) => {
      ...
      try {
        const res = await fetch(`${API_BASE}/lookup_booking.php`, {
          method: 'POST',
          headers: jsonHeaders(),
          body: JSON.stringify({ bookingCode, fullName })
        });
        const data = await res.json();
...
async function holdSeatOnServer(seatId, btn) {
  btn.disabled = true;
  try {
    const res = await fetch(`${API_BASE}/hold_seat.php`, {
      method: 'POST',
      headers: jsonHeaders(),
      body: JSON.stringify({ flightId: currentBooking.selectedFlight.id, seatNumber: seatId })
    });
    const data = await res.json();
...
function releaseSeatHold(seatId) {
  ...
  fetch(`${API_BASE}/release_seat.php`, {
    method: 'POST',
    headers: jsonHeaders(),
    body: JSON.stringify({ flightId: currentBooking.selectedFlight.id, seatNumber: seatId })
  }).catch(() => {});
...
window.confirmChangeBooking = async function () {
  ...
  try {
    const res = await fetch(`${API_BASE}/change_booking.php`, {
      method: 'POST',
      headers: jsonHeaders(),
      body: JSON.stringify({ bookingId, newOutboundFlightId: selectedFlightId, newDepartureDate: newDate })
    });
    const data = await res.json();
    if (!data.success) {
