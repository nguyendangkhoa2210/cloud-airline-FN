...
// Dọn các lượt giữ ghế đã hết hạn + lấy ghế đang bị giữ (bên dưới, trong try/catch)

// Ghế đang bị NGƯỜI KHÁC giữ tạm (còn hạn, khác session hiện tại) cũng coi như
// "không khả dụng" để người đang xem không chọn trùng ghế trong lúc người kia
// đang điền form/thanh toán. Ghế do CHÍNH session này đang giữ vẫn hiển thị bình
// thường để họ tiếp tục thao tác (bỏ chọn/giữ lại) như cũ.
//
// Bọc try/catch: nếu quên chạy migration_seat_holds.sql thì bảng seat_holds chưa
// tồn tại — KHÔNG được để lỗi này làm sập luôn màn hình chọn ghế, chỉ đơn giản
// coi như chưa có ghế nào đang bị giữ tạm (tính năng giữ ghế tạm tắt).
try {
