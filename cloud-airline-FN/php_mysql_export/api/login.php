...
// Chống dò mật khẩu (brute-force): khoá theo IP sau 5 lần sai trong 60 giây
$rateId = get_client_ip();
check_rate_limit($pdo, 'login', $rateId, 5, 60, 300);
...
if (!$user || !password_verify($password, $user['password'])) {
    register_failed_attempt($pdo, 'login', $rateId, 5, 60);
    json_response(['success' => false, 'message' => 'Email hoặc mật khẩu không đúng!'], 401);
}
clear_rate_limit($pdo, 'login', $rateId);
